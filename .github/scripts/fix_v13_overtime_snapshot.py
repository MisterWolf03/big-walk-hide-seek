...
# If an already-running relocation came from the previous preview build, seed it with a token.
...
# Every newly-started relocation gets a fresh unique token.
...
# Reset local retry state whenever the match is reset.
s = once(
    s,
    ...
    "local match reset overtime state",
)
# There are two reset sites (standalone and room bridge), patch the second separately if present.
needle='''      relocationState={active:false,index:0,nextAt:relocationSettings.firstAt,startedAt:null,deadline:null,lastResult:""};resetLocalRelocationPlan();'''
if needle in s:
    s=s.replace(needle,'''      relocationState={active:false,index:0,nextAt:relocationSettings.firstAt,startedAt:null,deadline:null,lastResult:""};resetLocalRelocationPlan();localRelocationOvertimeSnapshot=null;localRelocationOvertimeSignalIndex=0;''',1)

# Show whether the Seeker is waiting for the snapshot versus actively displaying it.
