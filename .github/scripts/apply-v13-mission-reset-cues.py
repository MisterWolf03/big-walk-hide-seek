...
# Fire availability and accepted cues, but suppress mission-cancel when a full match reset drops time back to zero.
...
# Make both local and room resets explicitly wipe all transient mission state.
once('''  missionState={active:false,index:0,nextAt:missionSettings.firstAt,type:"crossZone",startedAt:null,lastResult:"",token:""};hiderPowerCharges=0;resetLocalHiderMissionPlan();\n''', '''  missionState={active:false,available:false,index:0,nextAt:missionSettings.firstAt,type:"crossZone",startedAt:null,lastResult:"",token:""};hiderPowerCharges=0;missionStartRequestPending=false;missionSchedulerError="";resetLocalHiderMissionPlan();\n''','local reset mission state')
# Same line occurs in room reset too; patch the remaining copy.
once('''      missionState={active:false,index:0,nextAt:missionSettings.firstAt,type:"crossZone",startedAt:null,lastResult:"",token:""};hiderPowerCharges=0;resetLocalHiderMissionPlan();\n''', '''      missionState={active:false,available:false,index:0,nextAt:missionSettings.firstAt,type:"crossZone",startedAt:null,lastResult:"",token:""};hiderPowerCharges=0;missionStartRequestPending=false;missionSchedulerError="";resetLocalHiderMissionPlan();\n''','room reset mission state')
